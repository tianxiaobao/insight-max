            ...
            <div class="big-nav">
                ...
                <dl class="style-item">
                    ...
                    <dd>
                        <div class="big-rule-box m-setting" v-show="chartMsg.yAxis.visible">
                            ...
                            <h3>辅助线</h3>
                            <setting-number name="数值" v-model="chartMsg.yplotLine.value" @change="updatePlotLines"></setting-number>
                            <setting-number name="线宽" v-model="chartMsg.yplotLine.width" @change="updatePlotLines"></setting-number>
                            <setting-color name="颜色" v-model="chartMsg.yplotLine.color" @change="updatePlotLines"></setting-color>
                        </div>
                    </dd>
                </dl>
                <dl class="style-item">
                    <dt>
                        <b class="unfold close"></b><h3>气泡大小</h3>
                    </dt>
                    <dd>
                        <div class="big-rule-box m-setting">
                            <div class="item clearfloat">
                                <span class="left">
                                    <el-tooltip effect="dark" placement="top">
                                        <div slot="content">从节点Value值中选择最小的节点，使用<br>minSize进行绘制，默认值：8%</div>
                                        <i class="question-icon"></i>
                                    </el-tooltip>
                                    最小值
                                </span>
                                <div class="right">
                                    <input type="number" min="0" class="min-input" placeholder="8" v-model="size.min" @change="updateMinSize"> <b>%</b>
                                </div>
                            </div>
                            <div class="item clearfloat">
                                <span class="left">
                                    <el-tooltip effect="dark" placement="top">
                                        <div slot="content">从节点Value值中选择最大的节点，使用<br>maxSize进行绘制，默认值：20%</div>
                                        <i class="question-icon"></i>
                                    </el-tooltip>
                                    最大值
                                </span>
                                <div class="right">
                                    <input type="number" min="0" class="min-input" placeholder="20" v-model="size.max" @change="updateMaxSize"> <b>%</b>
                                </div>
                            </div>
                        </div>
                    </dd>
                ...
                <dl class="style-item">
                    <dt>
                        ...
                        <div class="fun-show" @click.stop="showInLegend"><i class="check" :class="{checked: chartMsg.plotOptions.bubble.showInLegend}"></i>显示</div>
                    </dt>
                    <dd>
                        <div v-if="chartMsg.plotOptions.bubble.showInLegend">
                ...
                <dl class="style-item">
                    ...
                    <dd>
                        <div class="big-rule-box m-setting" v-for="(data, index) of chartMsg.s">
                            ...
                            <setting-color name="边框颜色" v-model="data.marker.lineColor" @change="updateSeries"></setting-color>
                        </div>
                    </dd>
                </dl>
...
import bubble from '@/store/modules/defaultFormat/bubble';
...
export default {
    ...
    mounted () {
        this.$nextTick(() => {
            ...
            this.legend.align = this.chartMsg.legend.align;
            this.legend.verticalAlign = this.chartMsg.legend.verticalAlign;
            ...
            this.size.max = this.chartMsg.plotOptions.bubble.maxSize;
            this.size.min = this.chartMsg.plotOptions.bubble.minSize;
            this.toggleItem();
            this.addPlotLine();
        });
    },
    ...
    methods: {
        initChartMsg (chartMsg) {
            this.chartMsg = $.extend(true, {}, bubble[0].chartMsg, chartMsg);
        },
        updateChart () {
        ...
        updateMaxSize () {
            this.size.max = Math.max(0, this.size.max);
            if(this.size.max) {
                this.chartMsg.plotOptions.bubble.maxSize = this.size.max;
            } else {
                delete this.chartMsg.plotOptions.bubble.maxSize;
            }
            this.updateChart();
            this.$events.emit('redraw_chart');
        },
        updateMinSize () {
            this.size.min = Math.max(0, this.size.min);
            if(this.size.min) {
                this.chartMsg.plotOptions.bubble.minSize = this.size.min;
            } else {
                delete this.chartMsg.plotOptions.bubble.minSize;
            }
            this.updateChart();
            this.$events.emit('redraw_chart');
        },
        addPlotLine () {
            ...
            if(!this.activeLayer.chart) return;
        ...
        showInLegend () {
            this.chartMsg.plotOptions.bubble.showInLegend = !this.chartMsg.plotOptions.bubble.showInLegend;
            this.updateChart();
        },
        ...
        addSerie () {
            ...
            if(this.chartMsg.series.length === 0) {
                serieData = bubble[0].sourceData.sData;
            }
            serieOpt = {
                value: index,
                name: `数据列${index}`,
            ...
            // this.chartMsg.series.push(serieOpt);
