...
<div class="setting">
    <div class="big-buildCon" >
        <div class="big-nav" id="m-setting">
            ...
            <dl>
                <dt>
                    <a href="#" @click.prevent="xOpen = !xOpen"><b class="unfold"></b><h3>X轴</h3></a>
                    <div class="fun-show" @click.stop="isShow(chartMsg.xAxis,'visible')"><i class="check" :class="{checked: chartMsg.xAxis.visible}"></i>显示</div>
                </dt>
            ...
            <dl>
                ...
                <dd :class="{ open: yOpen }">
                   ...
                   <div class="big-rule-box m-setting">
                       <h3>辅助线</h3> 
                       <setting-number name="数值" v-model="chartMsg.yAxis.plotLines[0].value" @change="updateChart()"></setting-number>
                       <setting-number name="线宽" v-model="chartMsg.yAxis.plotLines[0].width" @change="updateChart()"></setting-number>
                       <setting-color name="颜色" v-model="chartMsg.yAxis.plotLines[0].color" @change="updateChart()"></setting-color>
                   </div>  
                ...
                <dd :class="{ open: sOpen }">
                    <div v-for="(item,index) in chartMsg.extra.series" :class="{'g-series': index != chartMsg.extra.series.length-1}">
                       ...
                       <div class="big-rule-box m-setting">
                           ...
                           <setting-number name="宽度" v-model="item.lineWidth" @change="updateSeries()"></setting-number>
                           <setting-color name="颜色" v-model="item.color" @change="updateSeries()"></setting-color>

                           <div class="item clearfloat">
                                <span class="left">样式</span>
                                <div class="right">
                                    <select class="u-select w100" v-model="item.dashStyle" @change="updateSeries()">
                                        <option v-for="item2 in dashStyle" :value="item2">{{item2}}</option>
                                    </select>
                                </div>
                            </div>

                            <div class="item clearfloat" v-if="activeLayer.type!='percentArea'">
                                <span class="left">近曲线</span>
                                <div class="right">
                                   <i @click="setChartType(item)" class="check" :class="{checked: item.type == 'areaspline'}"></i>
                                </div>
                            </div>
                       </div>
                       ...
                       <div class="big-rule-box m-setting">
                           <h3>
                            圆点
                            <div class="fun-show" @click.stop="setDataLabels(item.marker)"><i class="check" :class="{checked: item.marker.enabled}"></i>显示</div>
                            </h3>
                            
                            <setting-color name="颜色" v-model="item.marker.fillColor" @change="updateSeries()"></setting-color>
                            <setting-number name="半径" v-model="item.marker.radius" @change="updateSeries()"></setting-number>
                       ...
                       <div class="big-rule-box m-setting">
                            ...
                            <div class="item clearfloat">
                                ...
                                <div class="right">
                                    <div class="piece">
                                        <select class="u-select w160" v-model="item.fillColorObj.type" @change="updateSeries()">
                                            <option value="pure">颜色填充</option>
                                            <option value="linear">渐变填充</option>
                                        </select>
                                    </div>


                                    <div class="piece" v-show="item.fillColorObj.type =='pure'">
                                        <set-color :type="item.fillColorObj" :color="item.fillColorObj.color" @change="setColor"></set-color>
                                    </div>
                                    
                                    <div class="piece" v-show="item.fillColorObj.type =='linear'">
                                        <div class="clearfloat">
                                            <i class="fl u-linear" v-bind:style="{background : ['-webkit-linear-gradient(top,'+item.fillColorObj.linear.from+','+item.fillColorObj.linear.to+')','-moz-linear-gradient(top,'+item.fillColorObj.linear.from+','+item.fillColorObj.linear.to+')']}"></i>
                                            <div class="g-ml24">
                                                <set-color :type="item.fillColorObj.linear" :color="item.fillColorObj.linear.from" @change="setColorFrom"></set-color>
                                            </div>
                                            <div class="g-ml24">
                                                <set-color :type="item.fillColorObj.linear" :color="item.fillColorObj.linear.to" @change="setColorTo"></set-color>
                                            </div>
                                        </div>
                                        <div class="g-pad5">
                                            <el-slider
                                                :max="360"
                                                :min="0"
                                                v-model="item.fillColorObj.linear.value"
                                                @change="updateSeries()"
                                                >
                                            </el-slider>
                                        </div>
                                        <div>
                                            <div class="fl g-w60 mr5">
                                                <input class="g-w90b" disabled type="text" value="0">
                                                <div class="u-numdes">最小值</div>
                                            </div>
                                            <div class="fl g-w60 mr5">
                                                <input class="g-w90b" type="number" @change="updateSeries()" v-model.number="item.fillColorObj.linear.value" min="0" max="360">
                                            </div>

                                            <div class="fl g-w60">
                                                <input class="g-w90b" disabled type="text" value="360">
                                                <div class="u-numdes">最大值</div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            setColorFrom (obj) {
                obj.type.from = obj.color;
                this.updateSeries();
            },
            setColorTo (obj) {
                obj.type.to = obj.color;
                this.updateSeries();
            }
        }
    };
